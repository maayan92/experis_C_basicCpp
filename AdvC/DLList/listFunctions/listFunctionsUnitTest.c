...
/*
static void PrintElement(void* _a)
{
	printf("%d \t",*(int*)_a);
}
*/
...
static int PromotesByCon(void* _element, void* _context)
{
...
{
	int arr[SIZE] = {1,6,3,2,9}, insertD = 4;
	ListItr itrBegin, itrEnd;
	List *list = CreateAndFill(arr,SIZE);
	...
	if(*(int*)ListItrGet(itrEnd) != *(int*)ListItrGet(ListItrFindFirst(itrBegin,itrEnd,EqualData,&insertD)))
	{	
		DLListDestroy(list,NULL);
		return FAILED;
	}
...
Result TestListItrForEach_Valid()
{
	int arr[SIZE] = {1,6,3,2,9}, insertD = 3;
	ListItr itrBegin, itrEnd;
	...
	if(*(int*)ListItrGet(itrEnd) != *(int*)ListItrGet(ListItrForEach(itrBegin,itrEnd,PromotesByCon,&insertD)))
	{	
		DLListDestroy(list,NULL);
		return FAILED;
	}
...
Result TestListItrForEach_NotExist()
{
	int arr[SIZE] = {1,6,3,2,9}, insertD = 4;
	ListItr itrBegin, itrEnd;
	List *list = CreateAndFill(arr,SIZE);
...
Result TestListItrForEach_Empty()
{
	int insertD = 2;
	List *list = DLListCreate();
	...
	if(ListItrEnd(list) != ListItrForEach(ListItrBegin(list),ListItrEnd(list),PromotesByCon,&insertD))
	{	
		DLListDestroy(list,NULL);
		return FAILED;
	}
...
{
	int arr[7] = {1,8,3,2,6,9,7}, arrAfter[7] = {1,8,9,6,3,2,7};
	ListItr itrBegin, itrEnd;
	...
	if(!checkResult(ListItrPrev(ListItrPrev(ListItrEnd(list))),arrAfter,1,5))
	{	
		DLListDestroy(list,NULL);
		return FAILED;
	}
