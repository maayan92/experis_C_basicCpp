...
{
	return getListItr(_begin,_end,_action,_context,true);/*TODO unitTest*/
}
...
void ListItrSort(ListItr _begin, ListItr _end, LessFunction _less)
{
	...
	while(!ListItrEquals(_begin,_end) && swapped)
	{
		...
		swapped = PushBackByDirection(firstItr,secondItr,_end,_less,getPrev);
		
		getPrev(&firstItr,&secondItr);
		
		swapped = PushBackByDirection(firstItr,secondItr,_begin,_less,getNext);
...
static size_t PushBackByDirection(ListItr _first, ListItr _second, ListItr _end, LessFunction _less, ptrNextPrev _getNextPrev)
{
	...
	{
		if(_less(ListItrGet(_first),ListItrGet(_second)))
		{
			Swap(_first,_second);
			swapped = 1;
		}
